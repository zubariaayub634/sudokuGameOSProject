...
struct Vertex {int x,y; Vertex(int a=0, int b=0):x(a),y(b){}};

vector<pthread_t> threadsCreatedForEachInvalidEntry[9][9];
...
struct SectionData
{
	...
	int fReason;
	int presentValues[9];
	vector<Vertex> invalidEntries;
	SectionData():vertices(NULL),fReason(0)
	{
		for (int i=0; i<9; i++) 
			presentValues[i]=0;
	}
...
/*w*/	sem_wait(&matrixMutualExclusion);
	...
	for (int i=0; i<9; i++)
	{
		...
		{
			matrix[sec->vertices[i][0]][sec->vertices[i][1]]=0;
			*valid=false;
		}
		else
		{
			(sec->presentValues[matrix[sec->vertices[i][0]][sec->vertices[i][1]]])+=1;
			if (sec->presentValues[matrix[sec->vertices[i][0]][sec->vertices[i][1]]] > 1 && matrix[sec->vertices[i][0]][sec->vertices[i][1]]!=9)
			{
				*valid=false;
				if (sec->presentValues[matrix[sec->vertices[i][0]][sec->vertices[i][1]]] == 2)
				{
					for (int j=0; j<i; j++)
					{
						if (matrix[sec->vertices[j][0]][sec->vertices[j][1]] == matrix[sec->vertices[i][0]][sec->vertices[i][1]])
						{
							matrix[sec->vertices[j][0]][sec->vertices[j][1]]=0;
						}
					}
				}
				matrix[sec->vertices[i][0]][sec->vertices[i][1]]=0;
